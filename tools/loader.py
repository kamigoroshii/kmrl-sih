...
def is_scanned_pdf(pdf_path, min_text_length=20, pages_to_check=3):
    """
    Returns True if the PDF is likely scanned (image-based), False if digital (extractable text).
    Checks the first few pages for extractable text.
    """
    doc = fitz.open(pdf_path)
    num_pages = len(doc)
    for page_num in range(min(pages_to_check, num_pages)):
